...
export function seedDemoData(db: PlannerDatabase): void {
  ...
  const alphaOpening = createCinematic(db, { projectId: alpha.id, name: 'Seq010 — Opening', jiraKey: 'ALPHA-EP1', targetDate: '2026-09-30', notes: '', paused: false });
  createLoq(db, {
    cinematicId: alphaOpening.id, disciplineId: animationDisc.id, jiraKey: 'ALPHA-101', type: 'L1',
    status: 'DONE', estimateDays: 5, committedStart: '2026-09-01', committedFinish: '2026-09-05', actualFinish: '2026-09-05', dodRef: 'Approved by director', paused: false,
  });
  const alphaL2 = createLoq(db, {
    cinematicId: alphaOpening.id, disciplineId: animationDisc.id, jiraKey: 'ALPHA-102', type: 'L2',
    status: 'IN_PROGRESS', estimateDays: 8, committedStart: '2026-10-05', committedFinish: '2026-10-16', actualFinish: null, dodRef: '', paused: false,
  });
  createLoqResource(db, { loqId: alphaL2.id, personId: ava.id, startDate: '2026-10-05', finishDate: '2026-10-16', fte: 1 });

  const alphaClimax = createCinematic(db, { projectId: alpha.id, name: 'Seq030 — Climax', jiraKey: null, targetDate: null, notes: 'VFX-heavy beat, still gathering shots.', paused: false });
  createLoq(db, {
    cinematicId: alphaClimax.id, disciplineId: animationDisc.id, jiraKey: null, type: 'L1',
    status: 'TODO', estimateDays: 20, committedStart: '2026-11-02', committedFinish: '2026-11-08', actualFinish: null, dodRef: '', paused: false,
  });
  createLoq(db, {
    cinematicId: alphaClimax.id, disciplineId: vfxDisc.id, jiraKey: null, type: 'Final',
    status: 'TODO', estimateDays: null, committedStart: null, committedFinish: null, actualFinish: null, dodRef: 'Awaiting previz lock', paused: false,
  });

  // Bravo's Cinematic: fully staffed, two people sharing one LOQ's window at different FTE shares.
  const bravoEstablishing = createCinematic(db, { projectId: bravo.id, name: 'Seq020 — Establishing', jiraKey: null, targetDate: '2026-11-15', notes: '', paused: false });
  const bravoL1 = createLoq(db, {
    cinematicId: bravoEstablishing.id, disciplineId: animationDisc.id, jiraKey: 'BRAVO-055', type: 'L1',
    status: 'IN_PROGRESS', estimateDays: 6, committedStart: '2026-10-12', committedFinish: '2026-10-19', actualFinish: null, dodRef: '', paused: false,
  });
